...
const piwikIsAlreadyInitialized = (): boolean => {
    ...
    for (let i = 0; i < window._paq.length; i += 1) {
        if (window._paq[i].includes('setSiteId')) {
            hasSiteId = true
        }

        if (window._paq[i].includes('setTrackerUrl')) {
            hasTrackerUrl = true
        }
...
export class PiwikStore {
    ...
    public constructor(rawOpts: IPiwikStoreOpts = {}) {
        ...
        const clientTrackerName = rawOpts.clientTrackerName ?? 'matomo.js'
        ...
        if (!alreadyInitialized) {
            ...
            if (typeof url !== 'string' || typeof siteId !== 'number' || !injectScript) {
                ...
                this.push = (arg: unknown[]): void => console.debug(`PIWIK.push():`, arg)
                // eslint-disable-next-line @typescript-eslint/unbound-method
                this.setUserId = noop
                // eslint-disable-next-line @typescript-eslint/unbound-method
     ...
     */
    public track(location: Location | {path: string} | Location & {basename: string}, documentTitle = document.title): void {
